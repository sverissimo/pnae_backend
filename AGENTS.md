...
- PATCH /atendimento/:atendimentoId/aprovar-sei — **admin-only** DETEC/SEI approval of the relatório. Same `atendimentoId`-keyed shape as the validation routes, but the capability gate is `isAdmin()` alone (else **403**) via `assertCanAprovarSei`, then visibility (`hasAccessTo` on `getAtendimentoAuthScope(atendimentoId)`, else **404**) — a no-op for admins today, kept for model symmetry / a future scoped DETEC perfil. Sets `at_atendimento.data_sei = today()` via gateway `PATCH /api/aprovarSei/:id`; empty body; busts the atendimento cache. The coordenador-validation precondition (`data_validacao` present) is enforced atomically **in the gateway** and surfaces as a **400 with the gateway's message preserved** (the `RestAPI.patchAtendimentoValidacao` helper now reads the gateway `{ error }` body, so the reason reaches the UI via `PlainTextExceptionFilter`).
- PATCH /atendimento/:atendimentoId/remover-aprovacao-sei — admin-only; same auth shape as aprovar-sei; sets `data_sei = null` via gateway `PATCH /api/removerAprovacaoSei/:id`. No precondition (admins can always undo). Downstream-stage locking (`data_see`/`dt_export_ok`) is client-side UX only in v1 — not server-enforced. Busts the atendimento cache.
- GET /atendimento/getArquivos?atendimentoId=&fileType=foto|relatorio — downloads a stored file for an atendimento (PDF when `fileType=relatorio`, image when `fileType=foto`). Both are query params; the controller validates `fileType` is exactly `foto` or `relatorio`. The REST client maps those semantic values to the **PNAE-supported** `tipo_arquivo` MIME set (`application/pdf` for `relatorio`; `image/jpeg`, `image/png`, `image/gif` for `foto`) before calling `GET /api/getArquivos`, which returns `{ arquivo }` — a text column that actually holds the binary payload. The DOCX/TIFF/`application/msword` probe branches were intentionally dropped: a DB check found the relatório artifact is always a PDF and the proof-of-visit always an image, with only ~3 stray `application/msword` rows treated as bad data (never fetched here). The gateway route `GET /api/getArquivos` lives on the emater server (added for this flow — see its `atendimentoRoutes`): it returns the **lowest-`idArquivo` active** file matching that exact `tipo_arquivo` as `{ arquivo }`, or `{ arquivo: null }` on a miss (a normal 200 that advances the client to the next MIME). So selection is deterministic ("lowest `idArquivo` first") both here and in the list `arquivos[]` metadata. `AtendimentoService.getArquivos` decodes it via [decode-arquivo.ts](src/modules/atendimento/utils/decode-arquivo.ts) (handles data-URI / `\x` hex bytea / raw base64, with magic-byte Content-Type sniffing so a `foto` resolves to JPEG vs PNG correctly), and the controller streams the bytes back with the detected `Content-Type`. Must sit above `@Get(':id')` in the controller so the static route isn't shadowed. Web-only; the storage encoding is a best-effort guess and may need revisiting if the gateway format differs.
- GET /atendimento/com-relatorio-manual — web-only paginated list of Demeter atendimentos that have a manual relatório, sourced from gateway GraphQL `atendimentosComRelatorioManual`. Query params: optional `pageSize` (default 200, clamped 1..1000) and optional opaque `cursor`. Pushes authenticated `req.user` scope upstream before pagination (admin/developer: no scope; coordenador regional: `id_usuario + id_reg_empresa`; staff: `id_usuario`; missing/unsupported user: 403). Enriches each row with cached município/regional names from `getMunicipiosEmater`, collapses `clientes[0]` / `usuarios[0]` for PNAE's table view, and stays additive to the mobile contract. Each row also carries `arquivos[]` (`{ idArquivo, tipoArquivo, nomeArquivo }`, ordered lowest `idArquivo` first) — file metadata loaded upstream via a batched `at_arquivo` child query after the keyset page ids are known (the `arquivo` binary is never selected there). `tipoArquivo` is the raw stored MIME so the frontend derives its actions without sniffing bytes; `nomeArquivo` is an app-generated presentation name (`foto_<id>` for images, else `relatorio_<id>`), never Demeter's `nome_arquivo`.
- GET /relatorios/manual?id=<base64urlToken> — web-only public/no-auth PDF view for a manual relatório. Backend user filtering happens before this, in the manual-relatório list. The `id` query param is an obfuscated public PDF token, not an auth token: frontend prepends 5 random decimal digits to the Demeter `atendimentoId`, base64url-encodes that value, and the backend decodes it, strips the 5-digit noise prefix, validates the remaining atendimento id, then builds the PDF. This is only friction against obvious browser-bar ID guessing, not access control. Serves the **combined manual PDF** (Phase 2 of [docs/plans/atendimentos-manuais-file-download-options.md](docs/plans/atendimentos-manuais-file-download-options.md)): the wkhtmltopdf perfil cover, then every stored relatório PDF's pages copied in, then every proof-of-visit photo as an A4 image page — both file groups ordered lowest `idArquivo` first. Assembly lives in [manual-pdf-assembler.ts](src/@pdf-gen/manual-pdf-assembler.ts) (`@cantoo/pdf-lib` for page copy/embed; `sharp` re-encodes photos, normalizing GIF→PNG and applying EXIF auto-orientation). Files come in one call via `AtendimentoService.getArquivosAtendimento` → gateway `GET /api/getArquivosAtendimento` (all active `at_arquivo` rows with binaries, id asc — additive route on the emater server; the single-file `GET /api/getArquivos` stays untouched for the raw-download path). Any stored MIME outside the PNAE-supported set (notably `application/msword`) → **400** clear unsupported-file error before any PDF work; a parse/embed failure on a valid-typed file → **500** naming the offending `idArquivo`. Missing perfil or an empty file set still degrades to a minimal cover PDF (with a "nenhum arquivo" note) instead of failing.
