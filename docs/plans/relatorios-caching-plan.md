...
Even with a 30s TTL, the explicit busts matter because a user editing a relatório-linked atendimento on the web will refresh the dashboard within seconds and expect the new state.
...
| **Stale atendimento data.** | 30s TTL + explicit busts in `AtendimentoService` mutation methods. |
...
| **Orphan atendimentoIds (logic-removed upstream).** | Tombstone with sentinel `__nil__`, 30s TTL. Counted as `tombstoned=N` in the log line. |
...
| Phase 2 — `CachedAtendimentoReader` (30s TTL, tombstone for upstream-absent IDs) | **Shipped.** Wired into `hydrateRelatorios`. |
